...
| 35 | Configuration Comparison Engine | Real — built as a real 2nd `comparison_type` on the existing Universal Comparison Engine (migration 052), not a separate engine. **Extended this pass (migration 053) with a real Approved Baseline / Environment Override / Intentional Difference / Approved Exception classification layer** (`classifyConfigFinding()`, the directive's own Section 42 decision tree) — a real difference is no longer automatically treated as a defect; only reclassified when a real, staff-approved baseline actually defines a rule for that key | Real — Configuration Snapshots section + mode toggle, reusing existing comparison UI components. **Plus**: Configuration Baselines management section (create/approve), optional baseline selector on the comparison form, the exact 9-status icon set from Section 43, and a real "Mark as Intentional" exception action per finding | Enforced (RBAC on new routes; secret-shaped values masked in every response, proven live alongside the new classification logic) | `universal-comparison-engine.test.ts` +5 then +7 (23 total) + 607/607 full regression | **BLOCKED_EXTERNAL_AUTH** (see Technology row note) — Browser-pane interactive verification performed instead: real deliberate 8-key diff against a real approved baseline matched an independently-predicted classification exactly (expected difference / approved override / unapproved difference / approved exception / match / missing / extra / plain mismatch), plus a full "Mark as Intentional" round trip verified both in the UI and directly against the database | N/A | `configuration_comparison_test_1`, `configuration_baseline_test_1` | **PASS_WITH_RISKS** | Capped per the AUTHENTICATED PLAYWRIGHT EVIDENCE RULE (feature itself genuinely correct and tested); manual snapshot entry only (no live file-import/discovery yet); baseline classification applies to value-differences on shared keys only (missing/extra not baseline-reclassified in v1); single-level baseline only (no full Global→App→Env→Deployment inheritance, Section 40); no simultaneous multi-baseline comparison (Section 41); no baseline change-impact detection (Section 46); not yet wired into Database Schema comparison (Section 47's full reuse is a real, disclosed fast-follow); only "Mark as Intentional" built so far — Create Gap/Remediate/Use Baseline/Apply/Preview/Request Approval (Section 44) not yet built |
...
- **PASS_WITH_RISKS**: 16 engines (Configuration Comparison Engine, row
  #35 — extended this pass with the real Approved Baseline / Environment
  Override / Intentional Difference / Approved Exception classification
  layer, `configuration_baseline_test_1` — remains capped at
  `PASS_WITH_RISKS` per the AUTHENTICATED PLAYWRIGHT EVIDENCE RULE; status
  label unchanged from `configuration_comparison_test_1`, only the row's
  own detail was enriched)
