...
export async function apiRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/categories', async (req, reply) => { sendResult(reply, await catSvc.create(req.body as any), 201); });
  server.put('/categories/:id', async (req, reply) => { sendResult(reply, await catSvc.update((req.params as any).id, req.body as any)); });
  ...
  // Templates (Admin)
  server.post('/admin/templates', async (req, reply) => { sendResult(reply, await tmplSvc.createTemplate(req.body as any), 201); });
  ...
  server.post('/admin/templates/:id/attributes', async (req, reply) => { sendResult(reply, await tmplSvc.addAttribute({ ...(req.body as any), templateId: (req.params as any).id }), 201); });
  ...
  server.post('/items', async (req, reply) => { sendResult(reply, await itemSvc.create(req.body as any), 201); });
  server.put('/items/:id', async (req, reply) => { sendResult(reply, await itemSvc.update((req.params as any).id, req.body as any)); });
  ...
  server.post('/comparisons', async (req, reply) => { sendResult(reply, await compSvc.create(req.body as any), 201); });
