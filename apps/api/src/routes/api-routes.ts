...
export async function apiRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.get('/categories/:slug', async (req, reply) => { const c = await safeRead(() => catSvc.getBySlug((req.params as any).slug), null); if (!c) return reply.status(404).send({ error: { code: 'not_found', message: 'Category not found' } }); reply.send(c); });
  ...
  server.get('/items/:slug', async (req, reply) => { const item = await safeRead(() => itemSvc.getBySlug((req.params as any).slug), null); if (!item) return reply.status(404).send({ error: { code: 'not_found', message: 'Item not found' } }); reply.send(item); });
