...
// RISK-014 triage pass (2026-08-24): the /oc/otp/send email template interpolates
// several caller-supplied fields (businessOwner, clientName, onboardingData.*)
// directly into an HTML email body sent, via AskABD's real domain, to a
// caller-chosen recipient. Escaping them here closes a real HTML-injection /
// phishing-content vector — distinct from (and in addition to) that route's
// separate Admin.Access RBAC fix in platform/rbac/rules.ts.
...
export async function operationsCenterRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/oc/otp/send', async (req, reply) => {
    ...
    // Build email content from onboarding data — every caller-supplied field is
    // HTML-escaped (see escapeHtml above); only `otp` and the server-computed
    // expiry timestamp are ever interpolated unescaped, since both are
    // generated by this handler itself, never by the caller.
    const servicesHtml = (onboardingData?.services || []).map((s: string) => `<li>${escapeHtml(s)}</li>`).join('');
    const html = `
      <h2>AskABD Enterprise Client Onboarding Verification</h2>
      <p>Hello ${escapeHtml(businessOwner)},</p>
      <p>Your organization <strong>${escapeHtml(clientName)}</strong> has been registered for onboarding with AskABD.</p>
      <h3>Organization Information</h3>
      <ul>
        <li><strong>Company:</strong> ${escapeHtml(onboardingData?.companyName || clientName)}</li>
        <li><strong>Industry:</strong> ${escapeHtml(onboardingData?.industry || 'N/A')}</li>
        <li><strong>Country:</strong> ${escapeHtml(onboardingData?.country || 'N/A')}</li>
        <li><strong>Business Size:</strong> ${escapeHtml(onboardingData?.size || 'N/A')}</li>
        <li><strong>Support Model:</strong> ${escapeHtml(onboardingData?.supportModel || 'N/A')}</li>
        <li><strong>Criticality:</strong> ${escapeHtml(onboardingData?.criticality || 'N/A')}</li>
      </ul>
