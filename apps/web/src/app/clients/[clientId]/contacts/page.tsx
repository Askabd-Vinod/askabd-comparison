...
export default async function ClientContactsPage({ params }: PageProps) {
  ...
  if (!client) return <CapabilityPlaceholder title="Contacts" description="Contacts management for this client." />;

  const contacts = [
    { name: client.primaryContact.split('@')[0].replace('.', ' ').replace(/\b\w/g, l => l.toUpperCase()), role: 'Business Owner', email: client.primaryContact, phone: '+61 400 000 001', availability: 'Business Hours', type: 'executive' },
    { name: 'Technical Lead', role: 'Technical Owner', email: `tech.lead@${client.primaryContact.split('@')[1]}`, phone: '+61 400 000 002', availability: 'Business Hours', type: 'technical' },
    { name: 'Project Manager', role: 'Project Manager', email: `pm@${client.primaryContact.split('@')[1]}`, phone: '+61 400 000 003', availability: 'Business Hours', type: 'management' },
  ...
  const escalation = [
    ...
    { level: 'L3', contact: `${client.primaryContact} + hello@askabd.com`, response: '1 hour', scope: 'Critical with business impact' },
  ];
