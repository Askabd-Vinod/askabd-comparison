...
export default function OnboardClientPage() {
  ...
  async function completeOnboarding() {
    ...
    // Check for duplicate — if this org already exists and is in otp-sent, go to verify
    ...
    // 1. Save to localStorage (immediate UI update)
    ...
    }).catch(() => { /* API may not be running in dev — localStorage is the fallback */ });
    ...
    // 5. Create lifecycle state → organization-created → otp-sent
    ...
    // 6. Send OTP email via API — MUST succeed before navigating to /verify
    ...
    // 7. Navigate based on email send result
    if (emailSent) {
