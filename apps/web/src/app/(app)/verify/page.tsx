...
function VerifyPage() {
  ...
  useEffect(() => {
    ...
    if (clientIdParam) {
      // Load specific client by ID from URL param. localStorage is checked first for
      // an immediate paint (same-tab convenience only) — but if it's empty, this now
      // falls back to the real server lifecycle instead of declaring "not found".
      //
      // Found during the onboarding reload/multi-user audit: this previously trusted
      // localStorage ALONE. Opening this exact same, real, valid verify link in a
      // fresh browser, a different device, or after clearing site data showed "not
      // found" even though the real oc_lifecycle row existed server-side with status
      // 'otp-sent' — the DB was never consulted at all in that case.
      const lifecycleState = getLifecycleState(clientIdParam);
      if (lifecycleState) {
        setState(lifecycleState);
        loadClientData(clientIdParam);
      } else {
        fetchServerLifecycle(clientIdParam).then(serverState => {
          if (serverState) {
      ...
      // Fallback: find the most recent client in otp-sent state (no clientId in the
      // URL at all — genuinely same-tab-only convenience, there's no server-side
      // "most recent" concept to fall back to without a real client ID).
  ...
  function loadClientData(clientId: string) {
    // Real client record first — was previously localStorage-only (same bug class as
    // the client Overview page). Falls back to localStorage only if the API call fails.
    getClient(clientId).then(res => {
      if (res?.client) {
        const c = res.client;
        setClient({
          id: c.id, name: c.name, industry: c.industry, country: c.country, timezone: c.timezone,
          size: c.business_size, supportModel: c.support_model, criticality: c.criticality,
          businessOwner: c.primary_contact,
        });
        return;
      }
      loadClientDataFromLocalStorage(clientId);
    }).catch(() => loadClientDataFromLocalStorage(clientId));
  }

  function loadClientDataFromLocalStorage(clientId: string) {
    try {
  ...
  async function verifyOtp() {
    ...
    if (state) {
      ...
      logAuditEvent({
        ...
        action: 'otp_verified', actor: getStaffSession()?.identityId || 'unknown-staff',
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-12 text-center">
        ...
        <div className="mt-8 bg-green-50 border border-green-200 rounded-xl p-8">
          ...
          <p className="text-sm text-green-600 mt-2"><strong>{client?.name || state.organizationName}</strong> identity has been verified.</p>
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="mt-6 bg-white rounded-xl border p-6 shadow-sm">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">Organization: <span className="font-semibold text-gray-700">{client?.name || state.organizationName}</span></p>
