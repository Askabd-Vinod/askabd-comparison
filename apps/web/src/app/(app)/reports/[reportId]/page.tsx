...
export default async function ReportDetailPage({ params }: PageProps) {
  ...
  // Aggregate summary for the export files — same pattern as the report
  // cards on the /reports listing page (report-cards.tsx), which already
  // exports a summary row rather than the full table.
  const totalClients = mockClients.length;
  const avgScore = totalClients > 0 ? Math.round(mockClients.reduce((s, c) => s + c.platformScore, 0) / totalClients) : 0;
  const avgAvailability = totalClients > 0 ? (mockClients.reduce((s, c) => s + c.monitoring.availability, 0) / totalClients).toFixed(2) : '0';
  const totalDeployments = mockClients.reduce((s, c) => s + c.deployments.length, 0);
  const totalIncidents = mockClients.reduce((s, c) => s + c.incidents.length, 0);
  const slaCompliant = mockClients.filter(c => c.slaStatus === 'compliant').length;
    ...
    <div className="max-w-[1600px] mx-auto px-4 py-6 animate-in">
      ...
      <div className="bg-white rounded-xl border overflow-hidden">
        <div className="px-6 py-4 border-b flex items-center justify-between">
          ...
          {/* Previously two dead <button>s with no onClick at all — clicking
              them did nothing, no file, no error, no feedback. Found during
              the 2026-08-22 global UX audit. Now reuses the shared
              DownloadButton component (same one used on the /reports
              listing cards) instead of a one-off implementation. */}
          <div className="flex gap-2">
            <DownloadButton
              ...
              data={{ report: meta.name, clients: totalClients, avgScore, avgAvailability: `${avgAvailability}%`, totalDeployments, totalIncidents, slaCompliant: `${slaCompliant}/${totalClients}` }}
              className="text-xs bg-purple-50 text-purple-600 px-3 py-1.5 rounded font-medium hover:bg-purple-100 cursor-pointer"
            >
              Export PDF
            </DownloadButton>
            <DownloadButton
