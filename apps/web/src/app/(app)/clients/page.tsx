            ...
            {(['healthy', 'warning', 'critical', 'offline'] as const).map(healthStatus => {
              ...
              return (
                <div key={healthStatus} className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                  {/* Previously re-derived a second, byte-for-byte copy of
                      statusColor() (already imported above and used
                      elsewhere on this same page) instead of just calling
                      it — found during the 2026-08-22 global UX/duplicate-
                      component audit. */}
                  <div className={`${statusColor(healthStatus)} px-4 py-2 flex items-center justify-between`}>
