  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <TrendChart label="CPU Utilization" current={m.cpu} unit="%" data={generateTrend(m.cpu, 8, `${clientId}-cpu`)} warn={m.cpu > 80} />
        <TrendChart label="Memory Utilization" current={m.memory} unit="%" data={generateTrend(m.memory, 8, `${clientId}-mem`)} warn={m.memory > 80} />
      </div>

      <div className="grid md:grid-cols-2 gap-6 mb-6">
        <TrendChart label="Response Latency" current={m.latency} unit="ms" data={generateTrend(m.latency, 8, `${clientId}-latency`)} warn={m.latency > 200} />
        <TrendChart label="Database Latency" current={Math.round(m.latency * 0.6)} unit="ms" data={generateTrend(Math.round(m.latency * 0.6), 8, `${clientId}-db-latency`)} />
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <TrendChart label="API Response Time" current={Math.round(m.latency * 0.8)} unit="ms" data={generateTrend(Math.round(m.latency * 0.8), 8, `${clientId}-api-latency`)} />
        <TrendChart label="Throughput" current={Math.round(m.traffic / 60)} unit="req/s" data={generateTrend(Math.round(m.traffic / 60), 8, `${clientId}-throughput`)} />
      </div>
...
// Deterministic sample variance, not Math.random() — the same demo client's
// chart must show the same shape on every refresh (see deterministic-variance.ts).
function generateTrend(base: number, count: number, seed: string): number[] {
  return Array.from({ length: count }, (_, i) => Math.max(0, base + Math.round(seededVariance(`${seed}-${i}`, base * 0.3))));
}
