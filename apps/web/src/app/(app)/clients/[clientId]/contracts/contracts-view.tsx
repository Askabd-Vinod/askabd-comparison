...
export function ContractsView({ contracts, client, serviceCatalog }: { contracts: Contract[]; client: ClientData; serviceCatalog: CatalogService[] }) {
  ...
  function requestService(svc: CatalogService) {
    ...
    // who actually requested the service — found during the 2026-08-22
    // global UX/fabrication audit, same class of bug fixed elsewhere this
    // session (decision-transformation-service.ts, commercial-engagement-
    // service.ts, compliance-service.ts, workflow-automation-service.ts,
    // download-button.tsx).
    ...
    sendNotification({
      ...
      details: { action: `Service Added: ${svc.name}`, performedBy: actor, timestamp: new Date().toISOString(), impactLevel: 'None — new service provisioning' },
      recipients: [],
    }).catch(() => {});
  }
    ...
    <div>
      ...
      {showAddService && (
        <div className="mb-6 bg-purple-50 border border-purple-200 rounded-xl p-5">
          ...
          <p className="text-[10px] text-purple-600 mb-1">Select services to add to this client&apos;s engagement. Each request is logged to the audit trail immediately.</p>
          {/* The "Pending" list below is only tracked for this browser session
              — it resets on reload. The audit log entry above is real and
              permanent; this in-page list is a convenience view only. Being
              explicit about that rather than letting "Pending" imply a
              persisted, trackable status — found during the 2026-08-22
              global UX/fabrication audit. */}
          <p className="text-[10px] text-gray-400 mb-4">Note: the list below reflects this session only — refer to the audit trail for the permanent record.</p>
