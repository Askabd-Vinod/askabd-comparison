...
import { logAuditEvent, getClient } from '../../../../lib/operations-api';
...
import { DatabaseConnectionsManager } from '../../../../components/database-connections-manager';
...
export default function LifecycleJourneyPage() {
  ...
  const [relevantConnectors, setRelevantConnectors] = useState<{ connectorId: string; connectorName: string; category: string; classification: 'required' | 'optional'; status: string }[] | null>(null);
  // Real, multi-record database connection readiness for the Connector Configuration
  // step — replaces the old single fixed-field-set requirement model (see
  // database-connections-manager.tsx). null = not yet loaded (never treated as "ready").
  const [dbConnectionsReady, setDbConnectionsReady] = useState<boolean | null>(null);

  useEffect(() => {
    // Load from localStorage cache first (immediate render) — real client record below
    // is the source of truth; this is only a same-tab, same-session paint optimization.
    const ls = getLifecycleState(clientId);
    setState(ls);
    // Real client record — was previously localStorage-only, meaning a real client
    // opened by any staff member other than the one who ran the wizard, in another
    // browser, or after clearing site data, showed no name/header info at all.
    getClient(clientId).then(res => { if (res?.client) setClient({ name: res.client.name, businessOwner: res.client.primary_contact }); }).catch(() => {});
    const API_ROOT = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4200';
    fetch(`${API_ROOT}/api/v1/oc/clients/${clientId}/onboarding/requirements`)
  ...
  useEffect(() => {
    ...
    if (!allEmpty) return;
    // Use the real client record (fetched in the effect above) — not localStorage.
  ...
  }, [state?.status, requirements, client]);
  ...
  function allChecksComplete(): boolean {
    // Connector Configuration is gated by the real, multi-record database
    // connections list (see database-connections-manager.tsx) — at least one
    // connection must have genuinely passed a real connection test. Never the
    // generic requirements-readiness endpoint for this step (that model can't
    // represent "one or more of several possible connections").
    if (currentServiceReq?.serviceId === 'connector-configuration') return dbConnectionsReady === true;
  ...
  async function advancePhase() {
    ...
    try {
      const res = await fetch(`${API}/api/v1/oc/lifecycle/transition`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientId, event: currentPhase.nextEvent, actor: getStaffSession()?.identityId || 'unknown-staff', actorType: 'user' }),
      });
      ...
      if (res.ok) {
        ...
        if (data.success && data.lifecycle) {
          ...
          logAuditEvent({
            ...
            action: currentPhase.nextEvent, actor: getStaffSession()?.identityId || 'unknown-staff',
      ...
      try {
        ...
        const retryRes = await fetch(`${API}/api/v1/oc/lifecycle/transition`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ clientId: state!.organizationId || params.clientId, event: currentPhase.nextEvent, actor: getStaffSession()?.identityId || 'unknown-staff', actorType: 'user' }),
        });
        if (retryRes.ok) {
    ...
    <div className="space-y-6">
      ...
      {currentServiceReq && (
        <div className="bg-white rounded-xl border p-5">
          ...
          {/* Required Connectors — real, per-client relevance (ServiceRequirementMatrixService),
              never a static per-stage list. Configuration/testing live on the dedicated
              Connectors page, which already owns that real workflow. */}
          {relevantConnectors && relevantConnectors.length > 0 && (
            <div className="mt-4 pt-3 border-t">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-semibold text-gray-700">🔌 Required Connections</p>
              ...
              <div className="space-y-2">
                {relevantConnectors.map(conn => (
                  <div key={conn.connectorId} className="bg-gray-50 rounded-lg p-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold text-gray-800">{conn.connectorName}</p>
                      {conn.classification === 'required' && <span className="text-[7px] font-bold text-red-500 bg-red-50 px-1 rounded">REQ</span>}
                    </div>
                    <span className={`text-[9px] font-medium ${conn.status === 'connected' ? 'text-green-600' : conn.status === 'failed' ? 'text-red-600' : 'text-gray-400'}`}>
      ...
      {currentPhase ? (
        <div className="bg-white rounded-xl border p-5">
          ...
          {/* Guidance — Why is this service needed. Collapsed by default
              (Phase 3, 2026-08-20 compact redesign): this narrative content
              is real and unchanged, just no longer forced open by default —
              the single biggest contributor to this page's visual bulk on
              first load. <details> is native, keyboard-accessible, and needs
              no extra state. */}
          <details className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4 group">
            <summary className="text-[10px] font-bold text-purple-800 mb-0 cursor-pointer list-none flex items-center gap-1.5">
              <span className="transition-transform group-open:rotate-90">▸</span> 📋 Why is this step required?
            </summary>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[10px] mt-2">
          ...
          <div className="mb-4">
            ...
            {currentServiceReq?.serviceId === 'connector-configuration' ? (
              // Real, multi-record database connection management — a client can have
              // any number of connections (multiple databases, multiple environments,
              // multiple technologies). Replaces the old fixed single-connection field
              // set, which could never represent more than one database per client.
              <DatabaseConnectionsManager clientId={clientId} onReadinessChange={setDbConnectionsReady} />
            ) : readinessData && requirements.length > 0 ? (
              // Real fix (this pass): this used to render a flat, always-fully-expanded
              // "Service Requirements" status list here AND the fully interactive,
              // already-compact, per-requirement expand/collapse RequirementWorkspace
              // immediately below it — the exact same data shown twice, once as a
              // non-interactive duplicate. RequirementWorkspace already has its own
              // header summary (X/Y complete), its own real blockers → "Next Action"
              // panel, and collapses every requirement by default (only one opens at a
              // time) — so it alone is the canonical compact requirements UI now,
              // matching the same pattern Connector Configuration already uses.
          ...
          <div className="flex items-center gap-2">
            <button onClick={advancePhase} disabled={!allChecksComplete() || processing}
              ...
              {processing ? 'Processing...' : allChecksComplete() ? `${currentPhase.actionLabel} →` : currentServiceReq?.serviceId === 'connector-configuration' ? 'Add and test at least one connection to proceed' : readinessData ? `Complete required information to proceed` : `Complete all checks to proceed`}
            </button>
            {readinessData && currentServiceReq?.serviceId !== 'connector-configuration' && !allChecksComplete() && (
          ...
          {!allChecksComplete() && (
            <p className="text-[10px] text-amber-600 text-center mt-2">⚠ {currentServiceReq?.serviceId === 'connector-configuration' ? 'At least one database connection must pass a real connection test before you can proceed.' : readinessData ? `${readinessData.blockers?.length || 0} requirement(s) still needed. Save information above — this updates automatically.` : 'All validation checks must pass before proceeding.'}</p>
          )}
        </div>
      ) : (
      ...
      {/* Timeline — derived entirely from canonical statusMeta. Collapsed by
          default (Phase 3, 2026-08-20 compact redesign): the compact stepper
          in the header above already shows current position; this full
          step-by-step list (20+ rows) is real and unchanged, just tucked
          behind a toggle instead of always fully expanded. */}
      <details className="bg-white rounded-xl border p-5 group">
        <summary className="text-xs font-semibold text-gray-700 mb-0 cursor-pointer list-none flex items-center gap-1.5">
          <span className="transition-transform group-open:rotate-90">▸</span> Complete Lifecycle Timeline
        </summary>
