...
export default async function ServerDetailPage({ params }: Props) {
  ...
  if (!client) return <CapabilityPlaceholder title="Server Detail" description="Server tracking for this client." />;
  const serverIdx = parseInt(serverId, 10);
  if (isNaN(serverIdx) || serverIdx < 1 || serverIdx > client.infrastructure.servers) notFound();

  // Deterministic sample variance, not Math.random() — the same demo server
  // must show the same numbers on every refresh (see deterministic-variance.ts).
  const cpu = Math.round(client.monitoring.cpu + seededVariance(`${clientId}-${serverId}-cpu`, 10));
  const mem = Math.round(client.monitoring.memory + seededVariance(`${clientId}-${serverId}-mem`, 10));
  const disk = Math.round(client.monitoring.disk + seededVariance(`${clientId}-${serverId}-disk`, 5));
