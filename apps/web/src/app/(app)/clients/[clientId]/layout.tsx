...
export default async function ClientLayout({ children, params }: LayoutProps) {
  ...
  // Found during the final adversarial audit: ~30 client-scoped sub-pages under this
  // layout (applications, incidents, deployments, performance, etc.) render figures
  // computed from `mock-clients.ts` sample records with no indication to the viewer —
  // the `DemoDataBanner` component existed for exactly this purpose but was never
  // actually wired into any page. Fixed here, once, for every page this layout wraps,
  // rather than touching each sub-page individually. `clientId` only ever matches a
  // mock record for the ~20 fixed sample IDs baked into that file — every real,
  // onboarded client (a DB-generated ID) never matches, so this banner never appears
  ...
  if (!client) {
    return (
      <div className="max-w-[1600px] mx-auto px-4 py-6 animate-in">
        ...
        <div className="flex justify-end mb-3"><ClientSearchBox clientId={clientId} /></div>
        <ClientTabs clientId={clientId} />
        <div className="mt-4">
          <ClientPhaseNav clientId={clientId} status={lifecycleStatus} />
        </div>
        {isDemoClient && <div className="mt-4"><DemoDataBanner /></div>}
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 py-6 animate-in">
      ...
      <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-4">
          <ClientSearchBox clientId={clientId} />
          {healthScore !== null && (
            <Link href={`/clients/${clientId}/scorecard`} className="text-right hover:opacity-80 transition shrink-0">
